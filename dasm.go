...
type (
	disasm struct {
		...
		ir            uint16
		bus           AddressBus
		peekImmediate func(*Size) int
		readImmediate func(*Size) int
	}
...
func Disassemble(pc uint32, bus AddressBus) string {
	ir := uint16(bus.Read(pc, Word))
	pc += Word.size
	read := func(s *Size) int {
		result := bus.Read(pc, s)
		pc += s.align
		return result
	}
	write := func(s *Size) int { return bus.Read(pc, s) }
	return instructionTable[ir](&disasm{pc, ir, bus, read, write})
}
